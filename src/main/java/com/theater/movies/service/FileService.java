...
import static com.theater.movies.util.FileUtil.UPLOAD_FOLDER;
...
public class FileService {
    ...
    public byte[] getImage(String filename) {
        return FileUtil.getFile(Optional.ofNullable(filename)
                .orElseThrow(() -> new BadArgumentException("Filename is null!")));
    }
    ...
    public ResponseEntity<byte[]> prepareContent(String filename, String range) {
        ...
        var contentLength = String.valueOf((rangeEnd - rangeStart) + 1);
    ...
    public Long getFileSize(String fileName) {
        return Optional.ofNullable(fileName)
                .map(file -> Paths.get(UPLOAD_FOLDER, file))
